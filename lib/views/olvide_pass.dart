...
class RenewPassUser extends StatefulWidget {
  @override
  _RenewPassUserState createState() => _RenewPassUserState();
}

class _RenewPassUserState extends State<RenewPassUser> {
  ...
  Future updateUser(String clave, String dni, String empresa) async{
    print("DNI PARA OBTENER ID: "+ clave);
    String iddoc= "";
    var usuariosF = FirebaseFirestore.instance.collection("users").where("dni",isEqualTo: dni);
    QuerySnapshot users = await usuariosF.get();

    if(users.docs.isNotEmpty) {
      for (var doc in users.docs) {
        print("DATOS: " + doc.id.toString());
        iddoc = doc.id.toString();
      }
      final CollectionReference docUser = FirebaseFirestore.instance.collection("users");
      await docUser.doc(iddoc).update({'password': clave,
        'empresa': empresa});
    }
          ...
          setState(() {
            ...
            if(dataUser[0]["RPTA"] == "TRUE"){
              if(dataUser[0]["FECHA_NACIMIENTO"] == myControllerFecha.text) {
                ...
                updateUser(dataUser[0]["CLAVE"], myControllerUser2.text,
                    dataUser[0]["EMPRESA"]);
                            ...
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                Container(
                                  ...
                                  width:  MediaQuery.of(context).size.width / 1.3,
                                  child: const Text("EN CASO DE TENER INCONVENIENTES CON LA ACTUALIZACIÓN DE SU CONTRASEÑA, COMUNICARSE CON EL Nº CEL: 994601340", style: TextStyle(fontSize: 12),),
                                ),
                                const SizedBox(
                                  height: 16,
                                ),
          ...
          if(myControllerUser2.text.length == 8){
            ...
            showDialog(
               ...
               AlertDialog(
                ...
                content:
                const Text("¿Realmente desea actualizar con los datos ingresados?; recuerda tu clave para ingresar posteriormente con tu DNI y clave correctas"),
                actions: <Widget>[
                  TextButton(
