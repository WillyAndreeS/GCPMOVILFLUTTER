...
class CategoriaAcpClub extends StatefulWidget {
  String? categoriadet;
  CategoriaAcpClub({Key? key, this.categoriadet}) ;

  @override
  _CategoriaAcpClubState createState() => _CategoriaAcpClubState();
}

class _CategoriaAcpClubState extends State<CategoriaAcpClub> {
  late Size size;
  List categoriasF = [];
  List menus_ocultos = [];
  String? estado_menu1;
  ...
  Future<void> getCategoriasacpclub(categoriadet) async{

    var categoria = FirebaseFirestore.instance.collection("acpclub").doc("locales").collection("salud").where("categoria", isEqualTo: categoriadet);
...
setState((){
  ...
  void initState() {
    ...
    getCategoriasacpclub(widget.categoriadet!);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title:  Text(widget.categoriadet == "salud" ? "SALUD" : widget.categoriadet == "home" ? "HOGAR" : widget.categoriadet == "diversion" ? "DIVERSION" : "RESTAURANTES", style: TextStyle(fontFamily: "Schyler"),),
        leading: IconButton(
      ...
      body: categoriasF.isEmpty ? Center(child: CircularProgressIndicator()): Container(
